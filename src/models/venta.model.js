...
var Venta = function(venta){
  ...
  this.estado_venta             = venta.estado_venta;
...
dbConn.query("UPDATE ventas SET id_venta=?,fecha_venta=?,doc_identificacion_cliente=?,nombre_cliente=?,estado_venta=?,id_user=? WHERE id_venta = ?", [venta.id_venta,venta.fecha_venta,venta.doc_identificacion_cliente,venta.nombre_cliente,venta.estado_venta,venta.id_user, id], function (err, res) {
if(err) {
  console.log("error: ", err);
  result(null, err);
}else{
