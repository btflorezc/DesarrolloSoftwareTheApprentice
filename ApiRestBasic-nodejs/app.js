...
app.get("/get-product2/:descripcion_producto", async (request, response) => {
    const descripcion_producto = request.params.descripcion_producto;
    const consulta = 'SELECT id_producto, descripcion_producto, estado_producto, precio_producto FROM products WHERE descripcion_producto = \'' +descripcion_producto+ '\'';
    console.log(consulta);
    const [rows, fields] = await connection.execute(consulta);
    /* response.json({status:"OK"}); */
    console.log({ data: rows });
